...
//ListenInput listen input from the admin.
//Here are some helps:
//exit                                       --stop the server
//changeName   [UserName] [After]
//changeVIP    [UserName] [Bool]
//changeLevel  [UserName] [After]
//changeMoney  [UserName] [After]
func ListenInput() {
	for {
		var op string
		fmt.Scanln(&op)
		...
		if op == "exit" {
			SaveUserInfo()
			PrintLog("UserInfo Saved!")

			for _, conn := range loginMap {
				SendClientMsg(conn.conn, "exit", "")
			}

			os.Exit(0)
		} else if op == "changeName" {
			var username, after string
			fmt.Scanln(&username)
			fmt.Scanln(&after)
			userInfo := userMap[username]
			userInfo.Name = after
			userMap[username] = userInfo
			PrintLog("Name changed to " + after)
		} else if op == "changeVIP" {
			var username, after string
			fmt.Scanln(&username)
			fmt.Scanln(&after)
			userInfo := userMap[username]
			...
			userMap[username] = userInfo
			PrintLog("VIP changed to " + after)
		} else if op == "changeLevel" {
			var username, after string
			fmt.Scanln(&username)
			fmt.Scanln(&after)
			userInfo := userMap[username]
			userInfo.Level, _ = strconv.Atoi(after)
			userMap[username] = userInfo
			PrintLog("Level changed to " + after)
		} else if op == "changeMoney" {
			var username, after string
			fmt.Scanln(&username)
			fmt.Scanln(&after)
			userInfo := userMap[username]
			userInfo.Money, _ = strconv.Atoi(after)
			userMap[username] = userInfo
			PrintLog("Money changed to " + after)
		} else if op == "save" {
			SaveUserInfo()
			PrintLog("UserInfo Saved!")
		}
